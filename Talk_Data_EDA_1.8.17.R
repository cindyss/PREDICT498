...
library(reshape2); library(bit64); library(showtext);library(extrafont); library(zoo)
...
# Write CSV in R
write.csv(mobile2, file = "C:\\Users\\user1\\Documents\\Northwestern University\\PREDICT 498\\Data\\mobile2.csv")
...
find_users = setkey(mobile.train$device_id, NULL)
unique_device=  unique(mobile.train$device_id)
train.obs = nrow(unique_device)
mobile.train[,list(num=.N,prop=.N/train.obs),by=group][order(-num)]
options(repr.plot.width=6, repr.plot.height=3)
qplot(group, data=mobile.train, geom="bar")
